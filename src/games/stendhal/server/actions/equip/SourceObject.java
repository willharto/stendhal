...
class SourceObject extends MoveableObject {
	...
	public boolean moveTo(final DestinationObject dest, final Player player) {
		//if item is lucky charm and quantity is  > 1 and slot is keyring don't move
		if (item.getName().equals("lucky charm") && item.getQuantity() > 1 && dest.getContentSlotName().equals("keyring"))
		{
			return false;
		}
